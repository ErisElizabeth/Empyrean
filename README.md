...
- turn-in-place footwork for idle yaw rotation, using velocity-scaled blended cadence, diagonal step targets, damped planted-foot anchors, lower-body pelvis lead, and turn-only toe-in polish
...
| Stationary body turn cap                                                    | `stationaryTurnMaxYawRate` in `SOLO_TWEAKS.player`; `controlState.targetYaw` stores look intent while `controlState.yaw` remains the actual body/root yaw |
| Turn-in-place stepping                                                      | `turnInPlace*` values in `SOLO_TWEAKS`; implemented as a turn-only branch in `movementEngine.js`, including velocity intensity, cadence, blend, phase-speed, side-step, toe-yaw, and anchor-damping knobs |
...
- `0.2.25-alpha`: Raised the stationary body-yaw cap from 45 degrees per second to 60 degrees per second by changing `stationaryTurnMaxYawRate` to `Math.PI / 3`. The camera/look split, capped body catch-up, and `getTurnInPlaceFullAnimationVelocity()` intensity mapping remain the same, so the player should pivot with the same committed foot animation but a livelier body catch-up rate. Cache busters, README, and system map were updated after a checkpoint backup.
- `0.2.24-alpha`: Recalibrated turn-in-place animation intensity after the stationary yaw cap. `getTurnInPlaceFullAnimationVelocity()` now treats `stationaryTurnMaxYawRate` as the full-step animation rate when the body is capped, so a 45 degrees-per-second pivot still gives the feet full committed pivot-step lift, offsets, planted-anchor strength, toe yaw, and pelvis lead instead of fading into a held-foot slide. Cache busters, README, and system map were updated after a checkpoint backup.
- `0.2.23-alpha`: Added a first stationary body-yaw cap pass at 45 degrees per second through `stationaryTurnMaxYawRate`. Keyboard and RMB mouse-look now write immediate look intent into `controlState.targetYaw` while the actual body/root `controlState.yaw` chases it at the cap when there is no forward/back motion; camera follow uses `targetYaw` so the view remains responsive while the body catches up. Forward/back movement keeps the prior direct steering feel, with any leftover stationary yaw gap catching up through the existing higher `maxTurnVelocity`. Cache busters, README, and system map were updated after a checkpoint backup.
- `0.2.22-alpha`: Refined normal-speed turn cadence without limiting player yaw. `turnInPlaceCadenceIntensityFloor` now scales the turn phase-speed target from a slower gentle-pivot cadence up to the existing full cadence as `controlState.turnIntensity` rises, so slow pivots should no longer take tiny fast steps while keyboard-speed pivots keep the established rhythm. Cache busters, README, and system map were updated after a checkpoint backup.
- `0.2.21-alpha`: Added velocity-scaled turn animation intensity without limiting player yaw. `controlState.turnIntensity` maps smoothed `turnVelocity` from `turnInPlaceVelocityThreshold` to `turnInPlaceFullStepVelocity`, damps with `turnInPlaceIntensityDamping`, and multiplies the existing turn blend before bounce, pelvis lead, foot target offsets, lift, planted-anchor application, pitch polish, and toe yaw are applied. Slow pivots now use smaller turn steps while keyboard-speed pivots still reach full turn-in-place animation. Cache busters, README, and system map were updated after a checkpoint backup.
- `0.2.20-alpha`: Smoothed the turn-in-place branch by adding `controlState.turnBlendWeight` and `controlState.turnPhaseSpeed`. Turn stepping now eases in/out with `turnInPlaceBlendRiseDamping`, `turnInPlaceBlendFallDamping`, and `turnInPlacePhaseSpeedDamping`; bounce, pelvis lead, foot target offsets, lift, planted-anchor application, pitch polish, and toe yaw are scaled by the blend. Forward walk/run and walk preview clear the turn blend immediately so normal movement remains isolated. Cache busters, README, and system map were updated after a checkpoint backup.
- `0.2.19-alpha`: Replaced the generic walk-phase hip yaw during turn-in-place with a small direction-aware `turnInPlacePelvisYaw` lead. The lower body now participates in stationary pivots while the existing walk/run hip yaw remains unchanged for forward movement. Cache busters, README, and system map were updated after a checkpoint backup.
- `0.2.18-alpha`: Added a small turn-only foot-yaw polish after the existing ankle/foot pitch pass. `turnInPlaceFootYaw` toes the active foot into the current yaw direction using the same stride lift/plant/push-off markers that already drive the turn-in-place step, without changing the IK target solve or forward walk/run behavior. Cache busters, README, and system map were updated after a checkpoint backup.
- `0.2.17-alpha`: Fixed a turn-in-place runtime freeze introduced in `0.2.16-alpha` by threading `deltaTime` into `solveLegIK()` before the damped anchor refresh path reads it. Without that argument, entering the turn-only branch could throw a frame exception, leaving lil-gui responsive while keyboard/mouse movement, idle motion, walk preview, debug visibility, and sky-cycle updates appeared stopped because `requestAnimationFrame()` was no longer being scheduled. Cache busters, README, and system map were updated after a checkpoint backup.
- `0.2.16-alpha`: Refined the turn-in-place branch with a small diagonal side-step target keyed from the existing stride curve and turn direction, plus damped planted-foot anchor refresh during early stance. This keeps the turn-only footwork reading more like feet stepping around a pivot while preserving the existing IK solve and leaving forward walk/run behavior unchanged. Cache busters, README, and system map were updated after a checkpoint backup.
- `0.2.15-alpha`: Added the first measured turn-in-place stepping branch. Idle yaw now wakes the quaternion locomotion path when smoothed `turnVelocity` exceeds a threshold, advances a separate `turnPhase`, alternates foot lift with walk-style stride data, and feeds planted world-space foot anchors into the existing IK target solve so the feet no longer only sweep with root yaw. Forward walk/run constants and run blend behavior were not changed. Cache busters, README, and system map were updated after a checkpoint backup.
