...
instead of switching those targets directly from `controlState.isRunning`.
Build `0.2.14-alpha` added smoothed `controlState.turnVelocity` from wrapped
...
| `physics.js`        | Body mechanics/math         | Pure formulas for jump state, jump pose weights, stride phase, pelvis walk/run values, interpolation helpers. `movementEngine.js` consumes walk gait phase data from `getLegStrideValues()` and run gait phase data from `getRunStrideValues()` to blend walk/run foot targets, lift, and conservative ankle/foot phase pitch before IK solving. | Pure math only. No Three.js objects.                                                                                                               | None.                                                                                                                                                    | `main.js`, `movementEngine.js` walk/run foot-Z, lift target generation, and ankle/foot phase polish.    | KEEP. IK solver, bind-pose baseline, and controlled-arm ownership unchanged. |
...
| `rig.js`            | Rig defaults                | Default body proportions and GUI slider ranges. Current arm defaults are `upperArmLength = DEFAULT_RIG_HEIGHT * 0.17` (`0.7582`) and `forearmLength = DEFAULT_RIG_HEIGHT * 0.148` (`0.6601`); total arm reach is preserved. Current spine landmarks are `torsoY = DEFAULT_RIG_HEIGHT * 0.6121` (`2.7300`) and `chestY = DEFAULT_RIG_HEIGHT * 0.7287` (`3.2500`); `DEFAULT_RIG_HEIGHT`, `headY`, `neckY`, and `pelvisY` remain unchanged. | `DEFAULT_RIG_DIMENSIONS`, `DEFAULT_RIG_HEIGHT`, `RIG_DIMENSION_CONTROLS`.                                                                          | None.                                                                                                                                                    | `main.js`, `puppetShop.js` indirectly through package data; controlled-arm ownership and root/body fixture-zero ownership remain in `main.js` / the existing rig base-bind setup.      | KEEP. Rig-landmark constant change only.                                                    |
